...
Texture::Texture(const std::string filepath, const std::string filename, std::string texType, bool isPgm)
{
	...
	int width, height, nrComponents;
	stbi_set_flip_vertically_on_load(true);
	unsigned char* bytes = stbi_load(m_tempname.c_str(), &width, &height, &nrComponents, 4);
	if (bytes) {
		glBindTexture(GL_TEXTURE_2D, m_TextureID);
		...
		LOG(INFO, "Texture Pgm ID[%d] Type[%s] Width[%d] Height[%d] Name[%s]", m_TextureID, m_type.c_str(), width, height, filename.c_str());
		stbi_image_free(bytes);
	} else {
		LOG(ERRO, "Texture failed to load at path : %s", m_tempname.c_str());
		stbi_image_free(bytes);
