	...
	, m_objectModels(objectModels)
{
    ...
    loadPGMAndGenerateMap("res/map/nav.yaml", "res/map/nav.pgm", 0.05);
...
void Scene::GenerateGrid(int rows, int cols, float spacing, glm::vec3 color)
{
    ...
    for (int i = 0; i <= rows; ++i)
    {
        for (int j = 0; j <= cols; ++j)
        {
            ...
            m_verticesGird.push_back(Vertex{ glm::vec3(x, y, z), color ,glm::vec3(0.0f), glm::vec2(0.0f) });
        }
    }
...
void Scene::loadPGMAndGenerateMap(const std::string& yamlname, const std::string& pgmname, float spacing) 
{
    // load map info
    ...
    if (config["origin"]) {
        std::vector<float> originValues = config["origin"].as<std::vector<float>>();
    ...
    unsigned char* image = stbi_load(pgmname.c_str(), &width, &height, &channels, 0);
    ...
    m_verticesMap.clear();
    m_verticesMap.push_back(Vertex{glm::vec3(0.0f,               0.0f,               0.0f),glm::vec3(1.0f), glm::vec3(0.0f, 0.0f, 0.0f), glm::vec2(0.0f, 0.0f), glm::vec3(0.0f), glm::vec3(0.0f), {0}, {1.0f}});
    m_verticesMap.push_back(Vertex{glm::vec3(width * resolution, 0.0f,               0.0f),glm::vec3(1.0f), glm::vec3(0.0f, 0.0f, 0.0f), glm::vec2(1.0f, 0.0f), glm::vec3(0.0f), glm::vec3(0.0f), {0}, {1.0f}});
    m_verticesMap.push_back(Vertex{glm::vec3(0.0f,               height * resolution,0.0f),glm::vec3(1.0f), glm::vec3(0.0f, 0.0f, 0.0f), glm::vec2(0.0f, 1.0f), glm::vec3(0.0f), glm::vec3(0.0f), {0}, {1.0f}});
    m_verticesMap.push_back(Vertex{glm::vec3(width *resolution,  height *resolution, 0.0f),glm::vec3(1.0f), glm::vec3(0.0f, 0.0f, 0.0f), glm::vec2(1.0f, 1.0f), glm::vec3(0.0f), glm::vec3(0.0f), {0}, {1.0f}});
    ...
    m_mapTextureList.push_back(Texture(filepath, filename, "texture_diffuse", true));
}
