...
void SceneLayer::ShowModelPosition()
{
	...
	if (m_model.size() > 0)
	{
		...
		glm::quat rotationQuat = glm::angleAxis(m_model[m_selected]->objRotation.z, glm::vec3(0.0f, 0.0f, 1.0f));
		...
		ImGui::NewLine();
		if (ImGui::Button(u8"归零", ImVec2(buttonWidth, 25)))
		{
			m_model[m_selected]->objModel = glm::mat4(1.0f);
		}

		if (ImGui::Button(u8"添加点", ImVec2(buttonWidth, 25)))
		{
			Vertex vertex;
			vertex.position = m_model[m_selected]->objTranslation;
			vertex.position.z = 0.05;
			vertex.color = glm::vec3(1.0f, 0.0f, 0.0f);
			m_plan.push_back(vertex);
			m_sence->setPlan(m_plan);
		}

		if (ImGui::Button(u8"清空", ImVec2(buttonWidth, 25))) {
			m_plan.clear();
			m_sence->setPlan(m_plan);
		}
